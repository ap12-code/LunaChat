...
public class VelocityChannel extends Channel {
     ...
     * @param name チャンネル名
     */
    public VelocityChannel(String name) {
        super(name);
    }
    ...
    protected void sendMessage(ChannelMember player, String message,
        ...
        if ( isBroadcastChannel() ) {
            ...
            // 通常ブロードキャスト（全員へ送信）
            for ( Player p : LunaChatVelocity.getInstance().getProxy().getAllPlayers() ) {
                ChannelMember cp = ChannelMemberVelocity.getChannelMemberVelocity(p);
        ...
        if ( config.isOpListenAllChannel() ) {
            for ( Player p : LunaChatVelocity.getInstance().getProxy().getAllPlayers() ) {
                ChannelMember cp = ChannelMemberVelocity.getChannelMemberVelocity(p);
                if ( cp != null && cp.hasPermission("lunachat-admin.listen-all-channels")
    ...
    public int getOnlineNum() {
        ...
        if ( isBroadcastChannel() ) {
            return LunaChatVelocity.getInstance().getProxy().getPlayerCount();
        }
    ...
    public List<ChannelMember> getMembers() {
        ...
        if ( isBroadcastChannel() ) {
            ...
            for ( Player p : LunaChatVelocity.getInstance().getProxy().getAllPlayers() ) {
                mem.add(ChannelMemberVelocity.getChannelMemberVelocity(p));
            }
            return mem;
        }
