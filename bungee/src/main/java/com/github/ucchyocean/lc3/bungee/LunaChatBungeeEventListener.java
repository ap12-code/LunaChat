 ...
 * @author ucchy
 */
public class LunaChatBungeeEventListener implements Listener {
     ...
     */
    public LunaChatBungeeEventListener(LunaChatBungee parent) {
        this.parent = parent;
    ...
    public void onJoin(PostLoginEvent event) {
        ...
        if ( config.isShowListOnJoin() ) {
            for ( Component msg : getListForMotd(player) ) {
                player.sendMessage(serializer.serialize(msg));
            }
        }
    }
    ...
    public void onQuit(PlayerDisconnectEvent event) {
        ...
        for ( Channel channel : deleteList ) {
            LunaChat.getAPI().removeChannel(
                    channel.getName(), ChannelMember.getChannelMember(player.getUniqueId()));
        }
    }
    ...
    private void processChatEvent(ChatEvent event) {
        ...
        // 発言内容を非同期で処理する
        ProxyServer.getInstance().getScheduler().runAsync(LunaChatBungee.getInstance(), () -> processChat(ChannelMember.getChannelMember(((ProxiedPlayer) event.getSender()).getUniqueId()), event.getMessage()));
    ...
    private void forceJoinToForceJoinChannels(ProxiedPlayer player) {
        ...
        for ( String cname : forceJoinChannels ) {
            ...
            if ( channel == null ) {
                channel = api.createChannel(cname, ChannelMember.getChannelMember(player.getUniqueId()));
            }

            // チャンネルのメンバーでないなら、参加する
            ChannelMember cp = ChannelMember.getChannelMember(player.getUniqueId());
    ...
    private boolean tryJoinToGlobalChannel(ProxiedPlayer player) {
        ...
        if ( global == null ) {
            global = api.createChannel(gcName, ChannelMember.getChannelMember(player.getUniqueId()));
        }
     ...
     * @return リスト
     */
    private ArrayList<Component> getListForMotd(ProxiedPlayer player) {
     ...
     * @param message メッセージ
     */
    private void sendMessage(CommandSender reciever, Component message) {
        if ( message == null ) return;
        ChannelMember cm = ChannelMemberBungee.getChannelMemberBungee(reciever);
        if ( cm != null ) {
