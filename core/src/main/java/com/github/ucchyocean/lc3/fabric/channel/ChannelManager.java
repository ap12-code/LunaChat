...
public class ChannelManager implements LunaChatAPI {
    ...
    private static final String FILE_NAME_DEFAULTS = "defaults.yml";
    ...
    public void reloadAllData() {
        ...
        // デフォルトチャンネル設定のロード
        fileDefaults = new File(LunaChat.getDataFolder(), FILE_NAME_DEFAULTS);
        ...
        for ( String key : configHidelist.getKeys(false) ) {
            ...
            for ( String id : configHidelist.getStringList(key, new ArrayList<>()) ) {
                hidelist.get(key).add(LunaChat.getChannelMember(id));
            }
        }
    ...
    public Channel createChannel(String channelName, ChannelMember member) {
        ...
        Channel channel = LunaChat.getChannel(channelName);
    ...
    public ArrayList<ChannelMember> getHideinfo(ChannelMember player) {
        ...
        for ( String key : hidelist.keySet() ) {
            if ( hidelist.get(key).contains(player) ) {
                info.add(LunaChat.getChannelMember(key));
            }
        }
        return info;
    }
     ...
     * @param players ChannelMemberのリスト
     * @return IDのリスト
     */
