 ...
 */
package com.github.ucchyocean.lc3.bukkit.channel;
...
public class BukkitChannel extends Channel {
     ...
     * @param name チャンネル名
     */
    public BukkitChannel(String name) {
        super(name);
    }
    ...
    protected void sendMessage(
        ...
        if ( isBroadcastChannel() ) {
            ...
            for ( Player p : Bukkit.getOnlinePlayers() ) {
                ChannelMember cp = ChannelMemberBukkit.getChannelMemberBukkit(p);
        ...
        if ( isWorldRange() && player.isOnline() && player.getWorldName() != null ) {
            ...
            if ( Messages.noRecipientMessage("", "") != null && (
                    recipients.isEmpty() ||
                    (recipients.size() == 1 &&
        ...
        if ( config.isDisplayChatOnConsole() ) {
            LunaChat.getPluginLogger().info(message);
        }
    ...
    public List<ChannelMember> getMembers() {
        ...
        if ( isBroadcastChannel() ) {
            ...
            for ( Player p : Bukkit.getOnlinePlayers() ) {
                mem.add(ChannelMemberBukkit.getChannelMemberBukkit(p));
            }
            return mem;
        }
